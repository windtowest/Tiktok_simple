...
func (f *FollowServiceImp) getFollowing(userId int64) ([]*relation_gorm.User, error) {
	// 获取关注对象的id数组。
	ids, err := dao.NewFollowDaoInstance().GetFollowingIds(userId)
	// 查询出错
	if nil != err {
	...
	wg.Add(len)
	users := make([]*relation_gorm.User, len)
	i, j := 0, 0
	for ; i < len; j++ {
		if ids[j] == -1 {
			continue
...
// 从数据库查所有关注用户信息。
func getFollowing(userId int64) ([]*relation_gorm.User, error) {
	users := make([]*relation_gorm.User, 1)
	// 查询出错。
...
func (f *FollowServiceImp) getFollowers(userId int64) ([]*relation_gorm.User, error) {
	// 获取粉丝的id数组。
	ids, err := dao.NewFollowDaoInstance().GetFollowersIds(userId)
	// 查询出错
	if nil != err {
...
func (f *FollowServiceImp) getUserById(ids []int64, userId int64) ([]*relation_gorm.User, error) {
	len := len(ids)
	if len > 0 {
		len -= 1
	}
...
// 从数据库查所有粉丝信息。
func getFollowers(userId int64) ([]*relation_gorm.User, error) {
	users := make([]*relation_gorm.User, 1)
