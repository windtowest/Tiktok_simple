...
func GetFeedList(c *gin.Context) {
	...
	var userId int64
	userId = -1
	//tokenString := c.Query("token") token is not required here
	//todo:parser token to user_id
	...
	data, err := service.VideoList(userId) //获取videolist
	if err != nil {
		c.JSON(http.StatusOK, BaseResponse{1, err.Error()}) //error
		return
	}
